...
SOURCE="${HOME}/"
DESTINATION="/mnt/local_backup/home/$(whoami)/"
...
check_mount() {
    if ! mountpoint -q /mnt/local_backup 2>/dev/null; then
        err "/mnt/local_backup is not mounted."
...
cmd_restore() {
    ...
    if [[ "${1:-}" == "--delete" ]]; then
        ...
        [[ "$confirm2" =~ ^[Yy]$ ]] || { info "Aborted."; exit 0; }
        delete_flag="--delete"
    fi
